...
**Prices:**
Any tree-mode board (`hasPrices(sp)` = `treeOn(sp)`) carries prices on its **leaves**: a subtask, or a task with no subtasks, has a `price` (set from its menu's "Set price"). A task with subtasks shows the **rolled-up** sum of its subtasks (`taskPrice(task)`), not its own field, and offers no "Set price". The category counter appends the sum of its tasks' rolled-up prices; the subtask screen header shows the task's sum. `fmtPrice` uses `settings.currency` (Settings → Prices: ₽ $ € £ ¥). Subtask prices sync because `mergeBoards` carries the whole `subtasks` array.
