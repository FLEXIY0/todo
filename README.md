...
- **Пространства** — листай влево/вправо между To-Do, Wishlist и общим пространством с плавной 3D-анимацией переворота книжной страницы, полностью подконтрольной пальцу. Пространства можно переименовывать, выключать и добавлять новые в настройках
- **Общее пространство** — синхронизируется телефон-телефон напрямую через WebRTC (как торрент: сервер только знакомит устройства). Один тап «Invite» — код приглашения уже в буфере, на втором телефоне «Join» — и доски сливаются: объединение + побеждает последняя правка
- **История правок** — фиксируется *всё*: добавления, правки текста, выполнения, удаления, очистки. Любое удалённое возвращается одной кнопкой `↩`, правка текста откатывается. Глубина хранения настраивается прямо там (50 / 200 / 1000 / всё)
